...
Code to display Bscans obtained using J0-null method in FD-OCT
		based on mehtod (b) subtraction of interferograms
...
int main()
{
	...
	try 
	{
		...
		Mat data_y, data_yb, data_ydiff;
		Mat S, bscan;
		S = Mat::zeros(Size(numdisplaypoints,h), CV_64F);
		Sk = Mat::ones(Size(w,h),CV_16UC1);
		Mat positivediff, bscanlog, bscandb, tempmat, bscandisp, cmagI;
		...
		while (1)	//camera frames acquisition loop, which is inside the try
		{
			...
			// now read a signal image from the camera,  
			// and accumulate to Mat S
			...
			if(ret == 1)
			{
				data_ydiff = (data_y - data_yb) + 10000;
				oct.readInterferogram(data_ydiff);
